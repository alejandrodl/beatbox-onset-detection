...
frame_dev_absmeans = np.zeros((1,len(eval_window_lengths)))
frame_dev_absstds = np.zeros((1,len(eval_window_lengths)))
frame_dev_means = np.zeros((1,len(eval_window_lengths)))
frame_dev_stds = np.zeros((1,len(eval_window_lengths)))

accuracies = np.zeros((1,len(eval_window_lengths)))
precisions = np.zeros((1,len(eval_window_lengths)))
recalls = np.zeros((1,len(eval_window_lengths)))

all_thresholds_crossval = np.zeros((1,len(eval_window_lengths),num_crossval))
    ...
    for a in range(1):
        ...
        for n in range(len(eval_window_lengths)):
            ...
            for k in range(len(Pred)):
                ...
                if abs(min_value)<=eval_window_lengths[n]:
